%{
	...
	char* checkArithop1(char*,char*,char*);
	char* checkArithop2(char*,char*,char*); extern FILE *yyin;
...
S:	S Line
| Line;
Line:	label ':' ' ' Var '=' number '\n' {char *newReg=hasRegister($4);if(newReg==NULL){
...
newReg=(char*)malloc(sizeof(char)*10); sprintf(newReg,"R %d",register_count); reg[register_count].var=strdup($1); reg[register_count].registerName=strdup(newReg); register_count++;}
printf("MOV %s,#%s\n",newReg,$3);
